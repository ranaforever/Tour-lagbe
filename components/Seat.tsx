...
const Seat: React.FC<SeatProps> = ({ data, isSelected = false, selectionIndex, onClick }) => {
  if (data.isDisabled) {
    return (
      <div className="w-11 h-11 md:w-14 md:h-14 rounded-[12px] md:rounded-2xl border-2 border-dashed border-gray-200/50 bg-gray-50/30 flex items-center justify-center pointer-events-none opacity-30">
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`${getSeatColor()} w-11 h-11 md:w-14 md:h-14 rounded-[12px] md:rounded-2xl flex flex-col items-center justify-center text-[11px] md:text-[13px] font-black transition-all duration-200 shadow-md md:shadow-lg transform active:scale-95 relative`}
        title={data.isBooked ? `Booked: ${data.bookingInfo?.name || ''} (${data.id})` : `Seat ${data.id}`}
      >
        <span>{displayName}</span>
        {isSelected && typeof selectionIndex === 'number' && (
          <span className="absolute -top-1.5 -right-1.5 w-4 h-4 md:w-5 md:h-5 bg-white text-orange-600 rounded-full text-[9px] md:text-[10px] font-black flex items-center justify-center shadow-md border border-orange-200">
            {selectionIndex + 1}
          </span>
        )}
      </button>
