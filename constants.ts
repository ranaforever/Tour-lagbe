...
export const BUSINESS_INFO = {
  ...
  logo: "https://i.ibb.co/5XZxgVjj/Untitled-design.png",
...
/**
 * Dedicated Logo used exclusively for Ticket and Token printing/views.
 * Baki sob jaygay (app header, sidebar, login, etc.) BUSINESS_INFO.logo thakbe.
 */
export const DEFAULT_TICKET_LOGO = "https://i.ibb.co/m5gQ2PB5/Screenshot-2026-08-16-132920.png";

export const getTicketLogo = (): string => {
  try {
    const saved = localStorage.getItem('tl_ticket_logo');
    if (!saved || saved.startsWith('data:image/svg+xml') || saved.includes('<svg')) {
      return DEFAULT_TICKET_LOGO;
    }
    return saved;
  } catch (e) {
    return DEFAULT_TICKET_LOGO;
